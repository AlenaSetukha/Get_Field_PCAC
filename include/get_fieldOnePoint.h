...
#define _GET_FIELD_ONE_POINT_H_
...
//===================================================================================
//------------Электрическое поле идеального проводника в дальней
// зоне----------------
//===================================================================================
/**
...
void get_fieldOnePoint_idealFar(
    const double *x, const double *e0, const std::complex<double> k0,
    const double *k_vec, const std::vector<std::complex<double>[3]> &j_vec,
    const std::vector<double[CellPoints][3]> &cells, const Num_Par &num_par,
...
template <size_t CellPoints>
void get_fieldOnePoint_idealFarClar(
    const double *x, const double *e0, const std::complex<double> k0,
    const double *k_vec, const std::vector<std::complex<double>[3]> &j_vec,
    const std::vector<double[CellPoints][3]> &cells,
    const std::vector<double[3]> &norm, const Num_Par &num_parFar,
    const Num_Par &num_parNear, const double grid_step,
    std::complex<double> *field_E) {
  int num_frm = cells.size();
  field_E[0] = 0., field_E[1] = 0., field_E[2] = 0.;
...
//===================================================================================
//------------Электрическое поле идеального проводника в точках
// коллокаций-----------
//===================================================================================
/**
...
template <size_t CellPoints>
void get_fieldOnePoint_idealColloc(
    const double *x, const double *e0, const std::complex<double> k0,
    const double *k_vec, const std::vector<std::complex<double>[3]> &j_vec,
    const std::vector<double[CellPoints][3]> &cells,
    const std::vector<double[3]> &norm, const Num_Par &num_parFar,
...
//===================================================================================
//--------Электрическое поле в одной точке для одиночного
// диэлектрика----------------
//===================================================================================
/**
 ...
 * Расчет полного поля в одной точке с помощью операторов K[sigma, j](x),
 * R[sigma, j](x) для одного облучаемого диэлектрика: E_tot(x) = (i/(omega
 * eps1)) K1[Sigma, j_E](x) - R1[Sigma, j_M] + E_inc(x) -  внешняя область
 *      E_tot(x) = (i/(omega eps2)) K2[Sigma, j_E](x) - R2[Sigma, j_M] -
 * внутренняя область Здесь K_rot_rot - ближняя зона, без выделения сообенности,
 ...
 *      j_E/j_M [num_frm][3] - вектор эл/магн токов на ячейках в точках
 * коллокаций ed_param - электродинамические параметры задачи num_param -
 * численные параметры задачи field_E[3] - результат, поле в точке
 *
 *
 *
...
void get_fieldDielectricA2(const double *x,
  ...
  field_E[0] = 0., field_E[1] = 0., field_E[2] = 0.;
  //=Вычисление поля=
  std::complex<double> sum_K[3]{}, sum_R[3]{}, cur_res3[3]{}, j_E_min[3],
      j_M_min[3];
  std::complex<double> deg =
      std::complex<double>(0., 1.) /
  ...
  for (int i = 0; i < num_frm; i++) {
    ...
    K_rot_rot(j_E_min, x, cells[i], norm[i], num_param, ed_param.k[1],
              cur_res3);
    sum_K[0] += cur_res3[0];
    sum_K[1] += cur_res3[1];
    sum_K[2] += cur_res3[2];
