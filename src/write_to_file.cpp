        ...
        const std::vector<std::complex<double>[3]> &j)
{
    ...
    {
        throw std::runtime_error("Open " + filename + "_image.gv error: write_j_toFiles");
    }
    ...
    {
        fout_j_real << j[i][0].real() << " " << j[i][1].real() << " " << j[i][2].real() << std::endl;
        fout_j_image << j[i][0].imag() << " " << j[i][1].imag() << " " << j[i][2].imag() << std::endl;
    }
        ...
        const int n1, const int n2)
{
    ...
    if (!fout_u_abs.is_open() || !fout_u_image.is_open() || !fout_u_real.is_open())
    {
        throw std::runtime_error("Open " + filename + " error: write_field_toFiles");
    }
    ...
    for (int i = 0; i < n; i++)
    {
        fout_u_real << field_val[i][0].real() << " " << field_val[i][1].real() << " " << field_val[i][2].real() << std::endl;
        fout_u_image << field_val[i][0].imag() << " " << field_val[i][1].imag() << " " << field_val[i][2].imag() << std::endl;
        ...
        const ED_Par& ed_param)
{
    ...
    if (!fout_params.is_open()) {
        throw std::runtime_error("Open " + filename_out + " error: write_ED_Params_toFile");
    }
    ...
    for (int i = 0; i < ed_param.k_medium; i++)
    {
        ...
        fout_params << "    eps = " <<  ed_param.eps_d[i] * Constants::eps0<< std::endl;
    ...
    fout_params << "e0_H: " << ed_param.e0_H[0] << " " << ed_param.e0_H[1] << " " << ed_param.e0_H[2] << std::endl;
    fout_params << "e0_V: " << ed_param.e0_V[0] << " " << ed_param.e0_V[1] << " " << ed_param.e0_V[2] << std::endl;
    fout_params << "phi = " << ed_param.phi / (Constants::pi / 180.) << std::endl;
    fout_params << "teta = " << ed_param.teta / (Constants::pi / 180.)<< std::endl;
