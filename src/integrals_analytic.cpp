...
static double _Integral1Divr_inv(std::array<std::array<double, 3>, 3> const& ABC, std::array<double, 3> const& D)
{
...
{
    res = solid_angle(rut0[0], rut0[1], rut0[2], x) + solid_angle(rut0[2], rut0[3], rut0[0], x);
}
        ...
        std::complex<double>& ff)
{
    ...
    if (dist(x, y) < param.calc_dist)
    {
        ...
        ff = static_cast<std::complex<double>>(integral1Divr(rut0, x)) * Constants::pi_reverse;
        if (dist(x, y) < pow(10, -5))
        {
            ff += quadr_square(rut0[0], rut0[1], rut0[2], rut0[3]) * Constants::i_complex * param.k * Constants::pi_reverse;
        } else {
